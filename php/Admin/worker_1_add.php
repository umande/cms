<?php 
    ...
    ob_start();
     ...
     if(isset($_POST['worker_add'])){
        ...
        $idcmpy = mysqli_real_escape_string($conn, dataSanitizations($_POST['idcmpy']));

        $q1 = "SELECT id_company FROM company WHERE company_name = '$idcmpy'";
        $qur = mysqli_query($conn,$q1);
        $idw = mysqli_fetch_assoc($qur);
        $idw = intval($idw['id_company']);

        $q2 = "SELECT id_owner FROM owner WHERE id_company = $idw";
        $qur = mysqli_query($conn,$q2);
        $idw = mysqli_fetch_assoc($qur);
